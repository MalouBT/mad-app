...
const App: React.FC = () => {
    ...
    useEffect(() => {
        const init = async () => {
            try {
                ...
                setError("Kunne ikke initialisere Google-integration. Tjek konsollen for detaljer.");
                setIsLoadingData(false);
            }
        };
        init();
    ...
    if (!isDriveReady || isLoadingData) {
        return (
            <div className="flex flex-col items-center justify-center min-h-screen text-light" aria-live="polite">
                ...
                <p className="mt-4 text-lg">Indlæser data fra Google Drev...</p>
            </div>
        );
    }
    ...
    if (error) {
        return <div className="p-8 text-center text-red-400">
            ...
            <button onClick={() => window.location.reload()} className="mt-4 bg-accent text-primary font-bold py-2 px-4 rounded-lg">Prøv igen</button>
        </div>
    }
