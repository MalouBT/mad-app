...
declare namespace gapi {
    function load(apiName: 'client', callback: () => void): void;
    namespace client {
...
// VIGTIGT: Erstat disse placeholder-værdier med dine egne fra Google Cloud Console.
const CLIENT_ID = '422631206720-n40thk2581c9orh3cso3gktjiig8ne1g.apps.googleusercontent.com';
const API_KEY = 'AIzaSyA6gm5g6K8Dfo4qQ4BQ3JAXqFyF3nVL08E';
...
const gsiInit = () => new Promise<void>((resolve, reject) => {
    tokenClient = google.accounts.oauth2.initTokenClient({
        ...
        callback: (tokenResponse) => {
            // Denne callback håndteres af den, der kalder `requestToken`
        },
    });
    gsiInited = true;
    resolve();
...
export const requestToken = (onTokenReceived: () => void) => {
    ...
    // Sæt callback for når token er modtaget
...
export const findOrCreateDataFile = async (): Promise<{ fileId: string; data: DataBundle | null }> => {
    ...
    if (response.result.files && response.result.files.length > 0) {
        ...
        return { fileId: response.result.id!, data: initialContent };
    }
};
